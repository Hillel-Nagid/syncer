...
export default function Nav() {
  ...
  createEffect(() => {
    ...
    onCleanup(() => document.body.classList.remove("overflow-hidden", "md:overflow-auto"));
  });
  ...
  return (
    <>
          ...
          <div class="flex h-16 items-center justify-between">
            ...
            <div class="flex-shrink-0">
              ...
              >
                <div class="w-8 h-8 bg-gradient-to-br from-emerald-500 to-emerald-600 rounded-lg flex items-center justify-center shadow-lg group-hover:shadow-emerald-500/25 transition-shadow">
                  <Icon name="sync-icon" class="w-5 h-5 text-white" alt="Sync Icon" />
                </div>
            ...
            <div class="hidden md:block">
              <div class="ml-10 flex items-baseline space-x-8">
                <For each={navLinks}>
                  {(link) => (
                    <A href={link.href} class={`px-3 py-2 text-sm font-medium transition-all duration-200 rounded-md ${active(link.href)}`}>
                      {link.text}
                    </A>
                  )}
                </For>
      ...
      <MobileMenu isMobileMenuOpen={isMobileMenuOpen} closeMenu={closeMenu} navLinks={navLinks} />
    </>
  );
}
