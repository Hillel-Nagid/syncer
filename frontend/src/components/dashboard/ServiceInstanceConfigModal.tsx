...
import type {
	ExtendedServiceInstanceSyncSettings,
	ServiceInstance,
...
interface ServiceInstanceConfigModalProps {
	...
	onSave: (
		instanceId: string,
		instanceName: string,
		syncSettings: ExtendedServiceInstanceSyncSettings
	) => void;
	serviceInstance: () => ServiceInstance | null;
	customConfigSection?: JSX.Element;
	getServiceSpecificConfig?: () => ServiceSpecificConfig;
	initializeServiceSpecificConfig?: (
		config: ServiceSpecificConfig | undefined
	) => void;
	resetServiceSpecificConfig?: () => void;
}

export default function ServiceInstanceConfigModal(
	props: ServiceInstanceConfigModalProps
) {
	const [instanceName, setInstanceName] = createSignal<string>('');
	const [syncFrequency, setSyncFrequency] =
		createSignal<string>('Every 15 minutes');
	const [conflictResolution, setConflictResolution] =
		createSignal<ConflictResolution>('Keep both');
	...
	const initializeForm = () => {
		...
		if (instance) {
			...
			setConflictResolution(
				instance.syncSettings?.conflictResolution || 'Keep both'
			);

			// Initialize service-specific settings if handler provided
			const extended =
				instance.syncSettings as ExtendedServiceInstanceSyncSettings;
	...
	return (
		<Modal
			...
			onClose={handleClose}
			title={`Configure ${
				instance()?.instanceName || instance()?.name || 'Service'
			}`}
			size='lg'
		>
			<div class='space-y-6'>
				...
				<div class='space-y-4'>
					...
					<SelectField
						...
						options={[
							...
							{ value: 'Skip on conflict', label: 'Skip on conflict' },
						]}
					/>
				</div>
